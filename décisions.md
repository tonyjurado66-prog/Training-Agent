...
| 7.8 | Si claude.ai ne donne pas accès à Claude à la page (constaté avec un compte apprenant gratuit : capacité indisponible dès le chargement), la page propose **« Copier le tuteur de cette formation »** : le prompt complet (méthode + formation + profil) est copié pour être collé dans une nouvelle conversation claude.ai. | Une conversation claude.ai classique fonctionne avec tous les comptes, y compris gratuits. On perd les quiz à boutons et le suivi « Mon parcours », mais l'apprenant n'est jamais bloqué. | `instructions(true)` et `banner()` dans `tuteur.src.html`. |
